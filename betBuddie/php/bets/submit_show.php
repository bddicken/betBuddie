<?php
	...
	$bet_query2 = $db->prepare("
		SELECT winPayoff, placePayoff, showPayoff FROM RaceInstance WHERE RaceID = :rid AND HorseNumber = :hid");
	...
	else if($results2[0]['showPayoff'] > 0)
	{
	    $p = $_GET['amount']*(($results2[0]['showPayoff'])/2);
	    $new_value = $results[0][0] + $p; 
	    $rText = "Congratulations, you placed a good bet.  You won the show bet! <div class=\"winner\"></br>You Earned $" . number_format($p, 2) . "!!!</div>";
	} else 
	{
