<?php
	...
	$bet_query2 = $db->prepare("
		SELECT winPayoff FROM RaceInstance WHERE RaceID = :rid AND HorseNumber = :hid");
	...
	$new_value = "";
        if($results2[0][0] > 0) {
	    $change = (($_GET['amount'])* (($results2[0][0])/2) );
	    $new_value = $results[0][0] + $change;
	    $rText = "Congratulations, you placed a good bet!  <div class=\"winner\"></br>You Earned $" .  number_format( $change, 2 ). "!!!</div>";
	} else {
